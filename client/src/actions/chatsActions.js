...
export const selectChat = (id) => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.get(`${baseUrl}/api/v1/chat/${id}`, {
      headers: {
...
export const getRemoteStream = (stream) => (dispatch) => {
  dispatch({ type: "GET_REMOTE_STREAM", payload: stream });
};

export const endRemoteStream = () => (dispatch) => {
  dispatch({ type: "END_REMOTE_STREAM" });
};
